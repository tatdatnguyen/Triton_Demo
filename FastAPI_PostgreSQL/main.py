...
from db import save_inference, get_inferences, drop_inferences_table, init_db
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, json=payload) as response:
                ...
                if "outputs" in result:
                    ...
                    # Save input and output to PostgreSQL
...
async def get_all_inferences():
    try:
        ...
        if inferences:
            return {"inferences": [dict(row._mapping) for row in inferences]}
        else:
...
async def reset_inferences():
    try:
        await drop_inferences_table()
        return {"status": "inferences table dropped successfully"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
